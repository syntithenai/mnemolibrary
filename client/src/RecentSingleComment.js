    ...
    render() {
		...
		if (this.state.comments && this.state.comments.length > 0) {
			...
			let renderedComments = comments.map(function(comment) {
				...
				return  <span key={comment._id}>{comment.userAvatar} {joinWord} <i>"{shortComment}"</i>&nbsp;&nbsp;&nbsp;<div onClick={(e) => that.props.newCommentReply(comment)} className='btn btn-success'>{replyIcon}</div>&nbsp;<Link to='/recentcomments' className='btn btn-info'>...</Link></span>
			});
